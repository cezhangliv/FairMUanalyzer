...
void FairMUanalyzer::Init() {
    ...
    cbmsim_->SetBranchAddress("ReconstructedCalorimeterCluster.TotalEnergy", &totalE_);
    cbmsim_->SetBranchAddress("ReconstructedCalorimeterCluster.ClusterEnergy", &clusterE_);
...
void FairMUanalyzer::SaveResults() {
    ...
    */
    ...
    TCanvas* c50212 = new TCanvas(Form("c50212_%s", outputPrefix_.Data()), "c_totalE", 600, 400);
    c50212->Divide(4,3);
    c50212->SetLogy();
    h_totalE->Draw();
    c50212->SaveAs(Form("%s_c50212_h_totalE.pdf", outputPrefix_.Data()));

    TCanvas* c50213 = new TCanvas(Form("c50213_%s", outputPrefix_.Data()), "c_clusterE", 600, 400);
    c50213->Divide(4,3);
    c50213->SetLogy();
    h_clusterE->Draw();
